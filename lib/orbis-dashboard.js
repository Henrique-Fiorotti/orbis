...
import { maquinaTemSensores } from "@/lib/maquinas-table"
...
export function getMaquinasPorCriticidade(maquinas) {
  return ["ALTA", "MEDIA", "BAIXA"].map((criticidade) => {
    ...
    return {
      ...
      operando: maquinasDaFaixa.filter((maquina) => maquinaTemSensores(maquina) && maquina.status === "OK").length,
      emAlerta: maquinasDaFaixa.filter((maquina) => maquinaTemSensores(maquina) && maquina.status !== "OK").length,
      semSensor: maquinasDaFaixa.filter((maquina) => !maquinaTemSensores(maquina)).length,
    }
  })
}
