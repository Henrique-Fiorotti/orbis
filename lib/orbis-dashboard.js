...
import { getMaquinaStatusExibicao, maquinaTemSensores } from "@/lib/maquinas-table"
...
export function normalizeHistoricoIntegridadeItem(item, index = 0, maquina = null) {
  ...
  const integridade = toNumber(item.integridade ?? item.saude ?? item.healthScore ?? item.integridadeMedia, NaN)
  ...
  return {
    ...
    maquinaId: item.maquinaId ?? item.maquina?.id ?? maquina?.id ?? null,
    date: dateKey,
    timestamp,
...
export function getIntegrityTrendDataFromHistories(machineHistories, maquinas = [], referenceDateValue) {
  /** @type {Map<string, Map<string, { timestamp: number, integridade: number }>>} */
  ...
  for (const entry of machineHistories) {
    ...
    for (const point of entry.historico) {
      ...
      const values = valuesByDate.get(point.date) ?? new Map()
      const current = values.get(maquinaKey)

      if (!current || point.timestamp >= current.timestamp) {
        values.set(maquinaKey, {
  ...
  const entries = Array.from(valuesByDate.entries())
    .map(([date, values]) => {
      const readings = Array.from(values.values()).map((item) => item.integridade)
  ...
  if (entries.length > 0) {
    return fillIntegrityWindow(entries, referenceDateValue, 90)
  }
