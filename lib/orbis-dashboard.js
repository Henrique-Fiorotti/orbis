 ...
 * @returns {IntegridadeTrendChartDatum[]}
 */
export function getIntegrityTrendDataFromSnapshot(maquinas, referenceDateValue, days = 90) {
  ...
  const referenceDate = getReferenceDate(referenceDateValue ?? new Date())
  const windowSize = Math.max(1, Math.round(toNumber(days, 90)))

  return Array.from({ length: windowSize }, (_, index) => {
    const current = new Date(referenceDate)
    current.setUTCDate(referenceDate.getUTCDate() - (windowSize - 1 - index))
...
function getSingleMachineIntegrityTrendData(historico, maquina, referenceDateValue) {
  /** @type {Map<number, { date: string, timestamp: number, integridade: number, maquinas: number }>} */
  ...
  for (const point of Array.isArray(historico) ? historico : []) {
    ...
    valuesByTimestamp.set(point.timestamp, {
      date: point.date,
      timestamp: point.timestamp,
      integridade: point.integridade,
      maquinas: 1,
  ...
  const entries = Array.from(valuesByTimestamp.values())
    .sort((a, b) => a.timestamp - b.timestamp)
