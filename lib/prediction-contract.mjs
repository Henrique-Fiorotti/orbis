...
const NonNegativeIntegerSchema = z.preprocess(
  (value) => Math.max(0, Math.round(toNullableNumber(value) ?? 0)),
  z.number()
)
...
const PredictionModelMetadataSchema = z.preprocess(
  (value) => getPredictionModelMetadata(value),
  z.object({
    r2: z.number().nullable(),
    slope: z.number().nullable(),
    intercept: z.number().nullable(),
    pontosUsados: z.number().nullable(),
    janelaHorasCoberta: z.number().nullable(),
    ultimoPontoEm: z.string().nullable(),
  }).passthrough().nullable()
)

const PredictiveMaintenanceStateSchema = z.object({
  validasConsecutivas: NonNegativeIntegerSchema.catch(0),
  invalidasConsecutivas: NonNegativeIntegerSchema.catch(0),
  ultimaPredicaoEm: NullableTimestampSchema.catch(null),
  ultimaDataAgendada: NullableTimestampSchema.catch(null),
  ultimaPrevisaoManutencao: NullableTimestampSchema.catch(null),
  ultimoEstadoPredicao: z.preprocess(
    (value) => normalizePredictionCode(value),
    z.string()
  ).catch(""),
  ultimoMotivo: z.preprocess(
    (value) => (typeof value === "string" ? value.trim() : ""),
    z.string()
  ).catch(""),
  scoreConfianca: NullableNumberSchema.catch(null),
  criteriosAprovados: StringArraySchema.catch([]),
  criteriosReprovados: StringArraySchema.catch([]),
  bloqueadaPorPreventivaManual: BooleanSchema.catch(false),
  preventivaManualProximaId: NullableNumberSchema.catch(null),
  modeloIntegridade: PredictionModelMetadataSchema.catch(null),
})
...
export function normalizePredictiveMaintenanceState(raw) {
  ...
  const parsed = PredictiveMaintenanceStateSchema.safeParse({
    ...raw,
    ultimoEstadoPredicao: raw.ultimoEstadoPredicao ?? raw.estadoPredicao,
  })
  ...
  if (process.env.NODE_ENV !== "production") {
    console.warn("Payload de estadoPredicaoManutencao fora do contrato", parsed.error.flatten())
  }
