...
function normalizeManutencaoPrioridade(value) {
  ...
  if (["BAIXA", "MEDIA", "ALTA", "URGENTE"].includes(normalized)) {
    return normalized
  }
...
function normalizeManutencaoOrigem(value, alertaId) {
  ...
  return alertaId !== null ? "ALERTA" : "MANUAL"
}

function normalizeCumprimentoAgendamento(value) {
  const normalized = normalizeUppercase(value)

  if (["ANTECIPADA", "NO_PRAZO", "ATRASADA", "NAO_APLICAVEL"].includes(normalized)) {
    return normalized
  }
...
function normalizeMaquina(raw, index) {
  ...
  return {
    ...
    nome: normalizeString(raw.nome ?? raw.nomeMaquina ?? raw.name, `Máquina ${index + 1}`),
    setor: normalizeString(raw.setor ?? raw.area ?? raw.linha ?? raw.localizacao, "Sem setor"),
    tipo: normalizeString(raw.tipo ?? raw.tipoMaquina ?? raw.categoria ?? raw.modelo, "Máquina"),
...
function normalizeSensor(raw, index) {
  ...
  return {
    ...
    tipo,
    maquinaId,
    maquinaNome: normalizeString(raw.maquinaNome ?? raw.nomeMaquina ?? raw.maquina?.nome, "Sem máquina vinculada"),
...
function normalizeTecnico(raw, index) {
  ...
  return {
    ...
    nome: normalizeString(raw.nome ?? raw.nomeCompleto ?? raw.name ?? usuarioBase?.nome, `Técnico ${index + 1}`),
...
function normalizeManutencaoUsuario(source) {
  ...
  return {
    id,
    nome: normalizeString(source?.usuarioNome ?? source?.tecnicoNome ?? user?.nome ?? user?.name, id ? `Usuário ${id}` : "Usuário não informado"),
...
function normalizeManutencao(raw, index) {
  ...
  const maquina = raw.maquina && typeof raw.maquina === "object"
    ? normalizeMaquina(raw.maquina, index)
    : null
  const alerta = raw.alerta && typeof raw.alerta === "object"
    ? normalizeAlerta(raw.alerta, index)
    : null
  ...
  return {
    ...
    titulo: normalizeString(raw.titulo ?? raw.title ?? raw.nome, ""),
    prioridade: normalizeManutencaoPrioridade(raw.prioridade ?? raw.priority ?? raw.urgencia),
    ...
    criadoEm: normalizeString(raw.criadoEm ?? raw.createdAt ?? raw.dataCriacao, new Date().toISOString()),
    atualizadoEm: normalizeString(raw.atualizadoEm ?? raw.updatedAt, "") || null,
    ...
    cumprimentoAgendamento: normalizeCumprimentoAgendamento(raw.cumprimentoAgendamento ?? raw.cumprimento ?? raw.scheduleCompliance),
    diasDesvioAgendamento: toNullableNumber(raw.diasDesvioAgendamento ?? raw.desvioDias ?? raw.scheduleDeviationDays),
    metadataPredicao: raw.metadataPredicao && typeof raw.metadataPredicao === "object"
      ? raw.metadataPredicao
      : raw.predicao && typeof raw.predicao === "object"
        ? raw.predicao
        : null,
    alerta,
    maquina,
    maquinaNome: normalizeString(raw.maquinaNome ?? raw.nomeMaquina ?? maquina?.nome ?? alerta?.maquinaNome, maquinaId ? `Máquina ${maquinaId}` : "Máquina não informada"),
...
function normalizeAlerta(raw, index) {
  ...
  return {
    ...
    maquinaNome: normalizeString(
      ...
      maquinaId === null ? "Máquina não informada" : `Máquina ${maquinaId}`
    ),
    sensorId,
    sensorNome: normalizeString(
      raw.sensorNome ?? raw.nomeSensor ?? raw.sensor?.nome ?? raw.sensor?.tipo,
      sensorId === null ? "Sensor não informado" : `Sensor ${sensorId}`
    ),
