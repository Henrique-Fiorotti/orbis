 ...
 */
export async function askDashboardAi(pergunta, accessToken) {
  ...
  const payload = await requestDashboardJson(
    ...
    {
      method: "POST",
      body: { pergunta: normalizedQuestion },
    }
  )
...
function getLatestOperationalAlertDate(raw, fallback) {
  ...
  const latestEvent = eventos
    .map((evento) => normalizeString(evento.criadoEm ?? evento.createdAt, ""))
    .filter(Boolean)
...
function normalizeTecnico(raw, index) {
  ...
  return {
    ...
    nome: normalizeString(raw.nome ?? raw.nomeCompleto ?? raw.name ?? usuario?.nome, `Técnico ${index + 1}`),
...
function normalizeAdmin(raw, index) {
  ...
  return {
    ...
    telefone: normalizeString(raw.telefone ?? raw.celular ?? raw.whatsapp ?? raw.phone ?? usuario?.telefone, ""),
    role: "ADMIN",
    status: normalizeAdminStatus(raw.status ?? raw.estado ?? raw.situacao, raw.active ?? raw.ativo ?? usuario?.ativo),
    criadoEm: normalizeString(
      raw.criadoEm ?? raw.createdAt ?? raw.dataCriacao ?? raw.cadastradoEm ?? usuario?.createdAt,
      new Date().toISOString()
    ),
...
function normalizeAlerta(raw, index) {
  ...
  const tecnicoId = toNullableNumber(raw.tecnicoId ?? raw.tecnico?.id)
  ...
  const eventosOperacionais = getOperationalAlertEvents(raw)
  const ocorrencias = Math.max(
    toNumber(
      raw.ocorrencias ?? raw.totalOcorrencias ?? raw.quantidadeOcorrencias,
      eventosOperacionais.length || 1
    ),
    1
  )
  ...
  return {
    ...
    tecnicoId,
    tecnicoNome: normalizeString(raw.tecnicoNome ?? raw.tecnico?.nome, "") || null,
  }
}
