...
import { parseAlertApiRecord } from "./alert-contract.mjs"
import { parseMachineApiRecord } from "./machine-contract.mjs"
...
import { parseSensorApiRecord, parseSensorReadingApiRecord } from "./sensor-contract.mjs"
...
function normalizeSensorLeitura(raw) {
  const source = parseSensorReadingApiRecord(raw)
...
function normalizeMaquina(raw, index) {
  const source = parseMachineApiRecord(raw)
  ...
  return {
    ...
    dataInicioManutencao: normalizeString(
      raw.dataInicioManutencao ?? raw.data_inicio_manutencao ?? raw.maintenanceStartDate,
      ""
    ) || null,
    previsaoManutencao: normalizeString(
      raw.previsaoManutencao ?? raw.previsao_manutencao ?? raw.predictedMaintenanceAt,
      ""
    ) || null,
    dataFalha: normalizeString(
      raw.dataFalha ?? raw.data_falha ?? raw.previsaoFalha ?? raw.dataFalhaPrevista ?? raw.predictedFailureAt,
      ""
    ) || null,
...
function normalizeSensor(raw, index) {
  const source = parseSensorApiRecord(raw)
...
function normalizeAlerta(raw, index) {
  const source = parseAlertApiRecord(raw)
